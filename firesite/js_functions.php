...
var prev_info = "";  //report info from all created reports
...
function onLoadCreateReports(){
    ...
    if( prev_info != ""){
        ...
        for(var i = 0; i < report_info.length; i++){
            addReport(report_info[i]); //create header and <div> to place chart
        }
...
function storeVars(){
    ...
    switch(date_tab){
        ...
        case 3:
            ...
            var start_month = 2 * (Number(q) - 1) + Number(q);
            var end_month = 3 * Number(q);
            var start_day = 1;
    ...
    var start = formatDate(year, start_month, start_day);
    var end = formatDate(year, end_month, end_day);
...
function emptyChart(chart_loc){
    addElement("chart" + chart_loc, 'h2', "empty" + chart_loc, "<h2>There were no calls in this Date Range</h2>");
}
...
var date_tab = 4; // Indicator for which tab is active, start with 4:Year tab
